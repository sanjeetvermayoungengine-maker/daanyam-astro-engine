...
pub fn build_positions_sidereal_request_json(
    ...
    serde_json::to_string(&JsonSiderealPositionsRequest {
        datetime: JsonUtcDateTimeInput { kind: "utc", utc: datetime_utc },
...
pub fn build_chart_sidereal_request_json(
    ...
    serde_json::to_string(&JsonSiderealChartRequest {
        ...
        ayanamsa: "lahiri",
        gravitational_deflection,
        as_of: as_of_utc.map(|utc| JsonUtcDateTimeInput { kind: "utc", utc }),
        compact: options.compact,
        projection: map_projection(options.projection),
    })
...
#[napi]
pub fn parse_positions_response_json(json: String) -> String {
...
#[napi]
pub fn parse_positions_sidereal_response_json(json: String) -> String {
...
#[napi]
pub fn parse_chart_sidereal_response_json(json: String) -> String {
...
mod tests {
    ...
    #[test]
    fn napi_surface_reuses_core_math() {
        assert_eq!(normalize_angle(-30.0), 330.0);
    }

    #[test]
    fn napi_surface_exposes_projection_and_compact_options() {
    ...
    #[test]
    fn napi_positions_sidereal_request_helper_matches_http_shape() {
        let json = build_positions_sidereal_request_json(
            "2000-01-01T12:00:00Z".to_owned(),
            12.9716,
            77.5946,
            Some(920.0),
        ...
        assert_eq!(value["ayanamsa"], "lahiri");
    ...
    #[test]
    fn napi_chart_request_helper_matches_http_shape() {
        let json = build_chart_sidereal_request_json(
            ...
            Some(920.0),
            Some(false),
            Some("2000-01-02T00:00:00Z".to_owned()),
            Some(SiderealRequestOptions {
                compact: Some(true),
                projection: Some(ChartProjectionBinding::SiderealOnly),
            }),
    ...
    #[test]
    fn napi_positions_tropical_request_helper_matches_http_shape() {
        ...
        assert_eq!(value["compact"], true);
        assert_eq!(value["bodies"][1], "sun");
    }

    #[test]
    fn napi_positions_response_parser_round_trips_example_fixture() {
        let fixture = example_json("docs/examples/mobile_positions_tropical_compact.json");
        let round_trip = parse_positions_response_json(fixture.clone());
    ...
    #[test]
    fn napi_positions_sidereal_response_parser_round_trips_example_fixture() {
        let fixture =
            example_json("docs/examples/mobile_positions_sidereal_compact_sidereal_only.json");
        let round_trip = parse_positions_sidereal_response_json(fixture.clone());
        let parsed_fixture: Value = serde_json::from_str(&fixture).expect("fixture json");
        let parsed_round_trip: Value = serde_json::from_str(&round_trip).expect("round trip json");
        assert_eq!(parsed_fixture, parsed_round_trip);
    }
    ...
    #[test]
    fn napi_chart_sidereal_response_parser_round_trips_example_fixture() {
        let fixture = example_json("docs/examples/mobile_chart_compact_sidereal_only.json");
        let round_trip = parse_chart_sidereal_response_json(fixture.clone());
        let parsed_fixture: Value = serde_json::from_str(&fixture).expect("fixture json");
        let parsed_round_trip: Value = serde_json::from_str(&round_trip).expect("round trip json");
        assert_eq!(parsed_fixture, parsed_round_trip);
    }
